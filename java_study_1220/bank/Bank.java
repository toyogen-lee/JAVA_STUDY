...
public class Bank {    //계좌를 n개 생성
	//'클래스' 도 데이터 타입(레퍼런스 타입)이다.
	private BasicAccount[] accounts;    //BasicAccount 를 담는 배열
	...
	public void createAccount(String accountNumber) {    //계좌생성
	...
	public BasicAccount findAccount(String accountNumber) {    //계좌조회
		//클래스도 데이터타입이기 때문에 리턴타입으로 사용할 수 있음
		for(int i=0; i<accountCount; i++) {
			//equals : 문자를 비교할 때 사용하는 문자열 메소드
			if(accounts[i].getAccountNumber().equals(accountNumber)) {
				//account[i].getAccountNumber() : i번째에 저장된 BasicAccount의 getAccountNumber() 메소드 호출
